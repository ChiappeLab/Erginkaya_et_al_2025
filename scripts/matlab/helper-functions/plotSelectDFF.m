...
function f=plotSelectDFF(data1st,data2nd,neuronIndex,stimIndex,...
...
subIndex = 0;
[ha,~]=tight_subplot(length(neuronIndex),length(stimIndex)*2,...
    0.004,0.025,0.025);
...
for i = neuronIndex(1:length(neuronIndex)) % for each unique neuron-region
    ...
    limsX = [min(TimeAxisPD)+0.5 ceil(max(TimeAxisPD))-0.2];
%     limsX = [min(TimeAxisPD)-0.5 ceil(max(TimeAxisPD))];
    ...
    for j = stimIndex(1:length(stimIndex)) % for each OF stimulus
        ...
        if ~isempty(data1st{i,j}(:,iPlot)) % if there is imaging data
            ...
            switch plotType
                case 'Line'
                    plot(TimeAxisPD,data1st{i,j}(:,iPlot)); hold on;
                    plot(TimeAxisPD,nanmean(data1st{i,j}(:,iPlot),2),...
                        'color',colors(i,:),'LineWidth',lw);
                case 'Shaded'
                    nFlies=sum(~isnan(nanmean(data1st{i,j}(:,iPlot),1)));
                    h=shadedErrorBar(TimeAxisPD,...
                        nanmean(data1st{i,j}(:,iPlot),2),...
                        nanstd(data1st{i,j}(:,iPlot),0,2)/...
                        sqrt(nFlies));hold on
        ...
        subIndex = subIndex + 1;
        if  ~isempty(data2nd{i,j}(:,iPlot)) % if there is imaging data
            axes(ha(subIndex));
            % plot individual neurites, and mean across neurites, 2nd epoch
            switch plotType
                case 'Line'
                    plot(TimeAxisPD,data2nd{i,j}(:,iPlot)); hold on;
                    plot(TimeAxisPD,nanmean(data2nd{i,j}(:,iPlot),2),...
                        'color',colors(i,:),'LineWidth',lw);
                case 'Shaded'
                    nFlies=sum(~isnan(nanmean(data2nd{i,j}(:,iPlot),1)));
                    h=shadedErrorBar(TimeAxisPD,...
                        nanmean(data2nd{i,j}(:,iPlot),2),...
                        nanstd(data2nd{i,j}(:,iPlot),0,2)/...
                        sqrt(nFlies));hold on
                    set(h.mainLine,'Color',colors(i,:),...
                        'LineWidth',lw)
                    set(h.patch,'FaceColor',colors(i,:),...
